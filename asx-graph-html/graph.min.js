$(document).ready(function() {
  SimpleStockChart.prototype.draw = function() {
    try {
      ...
      $.ajax({
        ...
        var data = [];
        var dataIndex = response.data.length - 1;
  ...
  SimpleStockChart.prototype.init = function(x_i, x_ii) {
    try {
      ...
      var width = parseFloat(getComputedStyle(svg).width) - 70; 
      ...
      function mouseMoveTouch(event) {
        ...
        if (matches) {
          ...
          if (n) {
            ...
            var date = new Date(pointer.cache.date[i + x_i]);
            pointer3.textContent =
              ...
              ", " +
              date.getFullYear();
            var closeAt = parseFloat(pointer.cache.close[i + x_i]).toFixed(
              3
            );
            var vol = parseFloat(pointer.cache.volume[i + x_i]).toFixed(0);
